...
function SupportScreen({ navigate, activeScreen }) {
  ...
  const topics = [
    [Settings, 'Подключение и настройка', { background: '#EAF2FF', color: '#3B82F6' }],
    [Link, 'Подписка и оплата', { background: '#E6F6EC', color: '#2BB673' }],
    [Monitor, 'Устройства', { background: '#E6F6EC', color: '#2BB673' }],
    [Users, 'Реферальная программа', { background: '#FFE9DE', color: '#FF7A2F' }],
    [Shield, 'Безопасность и конфиденциальность', { background: '#EAF2FF', color: '#3B82F6' }],
    [Menu, 'Прочее', { background: '#EAF2FF', color: '#3B82F6' }],
  ];
...
function TopicRow({ icon: Icon, title, subtitle, tone, onClick }) {
  const tileStyle = typeof tone === 'object' ? { background: tone.background, color: tone.color } : undefined;
  const tileTone = typeof tone === 'object' ? 'custom' : tone;

  return (
    <button className="topic-row" onClick={onClick}>
      <IconTile tone={tileTone} style={tileStyle}><Icon size={22} /></IconTile>
      <div>
        <strong>{title}</strong>
        {subtitle && <p>{subtitle}</p>}
      </div>
